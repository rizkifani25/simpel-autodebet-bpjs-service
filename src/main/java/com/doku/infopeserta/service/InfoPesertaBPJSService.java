...
public class InfoPesertaBPJSService {
    ...
    public BundleModel getInfoPesertaBPJS(String noka) {
        ...
        pesertaAutodebet = this.restTemplate.postForObject(urlInternal, infoPesertaAutoDebetRequest,
                GetInfoPesertaAutodebetResponse.class, noka);

        if (pesertaBPJS.getMetadata().getErrorcode().equals("1")) {
        ...
        HttpEntity<TambahDataAutoDebetModel> tambahDataAutoDebetRequest = new HttpEntity<TambahDataAutoDebetModel>(
                insertNewData, headers);
        ...
        if (insertNewDataResponse.getMetadata().getErrorcode().equals("1")) {
